...
class CustomPPOTrainer_QSG(PPOTrainer, Trainer):
    ...
    def trans_text_to_token(self, messages_list):
        ...
        for messages in messages_list:
            if self.tokenizer.chat_template is not None:
                ...
                input_ids = self.tokenizer(conv.get_prompt(), return_tensors="pt").input_ids.to(target_device)
    ...
    def ppo_train(self, resume_from_checkpoint: Optional[str] = None) -> None:
        ...
        answers_path = os.environ.get(
            "MMOA_ANSWER_PATH",
            os.path.join(os.getcwd(), "data", "mmoa_hotpotqa_debug_train_top_k_docs.jsonl"),
        )
        logger.info("Loading PPO golden answers from {}".format(answers_path))
        ...
        for step in tqdm(range(max_steps), disable=not self.is_local_process_zero()):
            ...
            for idx in range(0, self.batch_size_1, self.config.mini_batch_size):
                ...
                retriever_url_template = os.environ.get("MMOA_RETRIEVER_URL_TEMPLATE")
                if retriever_url_template:
                    url = retriever_url_template.format(device=current_device)
                else:
                    url = os.environ.get("MMOA_RETRIEVER_URL", "http://127.0.0.1:8000/search")
                data = {
                    'questions': questions,
                    'N': 10
                }
                response = requests.post(url, json=data, timeout=float(os.environ.get("MMOA_RETRIEVER_TIMEOUT", "120")))
