...
from src.utils import GraphState, PlanExecState, RagState, StepTaskFormat, StepTaskState, PlanFormat, PlanSummaryFormat, PlanSummaryState, QAAnswerFormat, QAAnswerState, make_chat_openai, invoke_structured_with_retry
...
def build_plan_executor(retriever_tool = None):
    ...
    def single_task_execute(state: PlanExecState):
        ...
        if cur_task["type"] == "aggregate":
            ...
            response = invoke_structured_with_retry(chain, {"question": query}, task_name="aggregate_execute")
