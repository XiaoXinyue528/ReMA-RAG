...
from src.utils import PlanExecState, PlanSummaryState, StepTaskState, make_chat_openai, invoke_structured_with_retry
...
def task_define(state: PlanExecState):
    ...
    llm = make_chat_openai(temperature=0.3, api_key=API_KEY, max_retries=5)
    structured_llm = llm.with_structured_output(StepTaskFormat)
    chain = prompt | structured_llm

    # check stop or continue
    if len(state["step_output"]) >= len(state["plan"]) or (len(state["step_output"]) > 0 and str(state["step_output"][-1]["success"]).strip().lower() in ["no", "false"]):
        ...
        llm = make_chat_openai(temperature=0.0, api_key=API_KEY, max_retries=5)
        ...
        response = invoke_structured_with_retry(chain, {"plan": plan, "cur_step": cur_step, "memory": memory}, task_name="step_definer")
