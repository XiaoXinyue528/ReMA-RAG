...
class CorpusDatasetForEmbedding(Dataset):
    def __init__(self, doc_list):
        # 接收从 custom_corpus.jsonl 读出来的列表
        self.doc_list = doc_list
        self.n_docs = len(doc_list)

    def __len__(self):
        return self.n_docs

    def __getitem__(self, item):
        doc = self.doc_list[item]
        # 给文本加上 passage: 前缀，符合模型训练的格式要求
        text = f"passage: {doc['text']}"
        # 返回 int 类型的 doc_id 和文本
        return int(doc['doc_id']), text
